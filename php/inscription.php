...
// Header
vp_get_head('Inscription');
vp_get_nav();
vp_get_header('Inscription');
...
function vpl_form_processing($values, $errors) {
  ...
  vpl_check_name($errors, $values['utNom'], 'prenom', 50);
  $values['utPrenom'] = trim($_POST['prenom']);
  vpl_check_name($errors, $values['utPrenom'], 'nom', 50);

  // Vérification du jour/mois/année de naissance
  vpl_check_select($_POST['naissance_j'], 1, 31);
  vpl_check_select($_POST['naissance_m'], 1, 12);
  vpl_check_select($_POST['naissance_a'], 1920, 2020);
...
function vpl_print_form($values, $errors) {
  echo '<section>',
    '<h2>Formulaire d\'inscription</h2>',
      ...
      vp_print_form_errors($errors, 'Les erreurs suivantes ont été relevées lors de votre inscription :');
      ...
      echo '<form action="inscription.php" method="post">',
        '<table>';
          vp_print_table_form_input('Choisissez un pseudo', 'pseudo', htmlentities($values['utPseudo']), true, 'text', '4 caractères minimum');
          vp_print_table_form_radio('Votre civilité', 'radSexe', array('Mr', 'Mme'), $civilite, array('Monsieur', 'Madame'), true);
          vp_print_table_form_input('Votre nom', 'nom', htmlentities($values['utNom']), true);
          vp_print_table_form_input('Votre prénom', 'prenom', htmlentities($values['utPrenom']), true);
          vp_print_table_form_date('Votre date de naissance', 'naissance', 2020, 1920, $naissance_j, $naissance_m, $naissance_a);
          vp_print_table_form_input('Votre email', 'email', htmlentities($values['utEmail']), true);
          vp_print_table_form_input('Choisissez un mot de passe', 'passe1', '', true, 'password');
          vp_print_table_form_input('Répétez le  mot de passe', 'passe2', '', true, 'password');
          vp_print_table_form_checkbox(array('cbCGU', 'cbSpam'), array(1, 1), array(0, $values['utMailsPourris']), array('J\'ai lu et j\'accepte les conditions générales d\'utilisation', 'J\'accepte de recevoir des tonnes de mails pourris'), array(true, false));
          vp_print_table_form_button(array('submit', 'reset'), array('S\'inscrire', 'Réinitialiser'), array('btnInscription', ''));
        echo '</table>',
      '</form>',
    '</section>';
}
 ...
 */
function vpl_check_name(&$errors, $value, $field_name, $length) {
  if(empty($value)) {
 ...
 */
function vpl_check_select($value, $min, $max) {
